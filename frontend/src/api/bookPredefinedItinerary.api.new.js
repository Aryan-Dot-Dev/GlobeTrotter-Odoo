...
const bookPredefinedItinerary = async (predefinedItinerary) => {
    try {
        ...
        const stops = predefinedItinerary.highlights?.map((highlight, index) => ({
            city: highlight,
            country: 'India', // Assuming all destinations are in India
            arrival_date: new Date(Date.now() + (index * 24 * 60 * 60 * 1000)).toISOString().split('T')[0],
            departure_date: new Date(Date.now() + ((index + 1) * 24 * 60 * 60 * 1000)).toISOString().split('T')[0]
        })) || [];
